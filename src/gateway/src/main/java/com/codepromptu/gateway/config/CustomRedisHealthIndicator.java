...
public class CustomRedisHealthIndicator implements HealthIndicator {
    ...
    @PostConstruct
    public void init() {
        logger.info("=== GATEWAY CUSTOM REDIS HEALTH INDICATOR INITIALIZED ===");
        logger.info("Redis Connection Factory: {}", redisConnectionFactory.getClass().getSimpleName());
    }

    @Override
    public Health health() {
        logger.info("=== GATEWAY CUSTOM REDIS HEALTH CHECK CALLED ===");
        try {
            ...
            logger.info("Gateway Redis health check SUCCESS: {}", pong);
            return Health.up()
            ...
            logger.error("Gateway Redis health check FAILED: {}", e.getMessage());
            return Health.down()
